...
\subsection{Реализация}
Создан отдельный класс \textbf{many\_weight\_linear\_model}. Для SVM требуется реализовать подсчёт функции потерь и её градиента. Само проведение шагов снова оставлено на \textbf{scipy.optimize}. Метод, считающий функцию потерь - \textbf{svm\_lost(self, X, y, W, b)}, где под y подразумевается вектор номеров классов, имеющий значения от нуля до $M - 1$, W - вектор весов, b - вектор сдвигов(В разделе с многоклассовой классификацией нет надобности в упрощении, убирающем b). Реализация \textbf{svm\_lost(self, X, y, W, b)}:
